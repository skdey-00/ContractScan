...
function StackedBar({ red, amber, green }: { red: number; amber: number; green: number }) {
  ...
  return (
    <div className="flex rounded-full overflow-hidden h-3 w-full max-w-md bg-gray-100">
      {red > 0 && (
  ...
  return (
    <div>
      ...
      >
        ...
        <svg
          className={`w-3.5 h-3.5 text-gray-400 ml-auto flex-shrink-0 transition-transform duration-150 ${open ? 'rotate-180' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={2}
      ...
      {open && (
        <p className="text-xs text-gray-500 leading-relaxed pl-2 pb-2">
          {clause.summary}
        </p>
      )}
    </div>
